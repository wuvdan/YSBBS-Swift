...
extension HXNavigationController {
    ...
    @objc private func handleinteractivePopGesture(gesture: UIScreenEdgePanGestureRecognizer) {
        ...
        if gesture.state == .changed {
            navigationBar.tintColor = average(fromColor: fromVC.hx_tintColor, toColor: toVC.hx_tintColor, percent: coordinator.percentComplete)
        }
    }
...
extension HXNavigationController {
    ...
    func hx_updateNavigationBarTint(for viewController: UIViewController, ignoreTintColor: Bool = false) {
        ...
        UIView.setAnimationsEnabled(false)
        navigationBar.barStyle = viewController.hx_barStyle
        let titleTextAttributes = [
            NSAttributedString.Key.foregroundColor: viewController.hx_titleColor,
            NSAttributedString.Key.font: viewController.hx_titleFont
        ]
        navigationBar.titleTextAttributes = titleTextAttributes
        if !ignoreTintColor {
            navigationBar.tintColor = viewController.hx_tintColor
        }
        UIView.setAnimationsEnabled(true)
    }
