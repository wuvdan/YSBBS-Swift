...
class TopicListTableViewCell: UITableViewCell {
    ...
    private lazy var commentButton: UIButton = {
        let l = UIButton.init()
        l.setTitle(" 评论", for: .normal)
        l.setImage(kGetImage(imageName: "Comment"), for: .normal)
        l.setTitleColor(UIColor.lightGray, for: .normal)
        l.titleLabel?.font = UIFont.systemFont(ofSize: 11)
        l.addTarget(self, action: #selector(didTappedCommentButton), for: .touchUpInside)
        return l
    }()
    ...
    public var model: HomeTopicListModel {
        get {
            ...
            commentButton.setTitle(" \(newValue.commentNum > 0 ? "\(newValue.commentNum)" : "评论")", for: .normal)
            likeButton.setTitle(" \(newValue.likeNum > 0 ? "\(newValue.likeNum)" : "赞")", for: .normal)
        }
    }
    ...
    @objc func didTappedLikeButton() {
        if (didTappedLikeButtonBlock != nil) {
            didTappedLikeButtonBlock!(model)
        }
    }
